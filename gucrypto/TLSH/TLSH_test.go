...
//
// This source has been partially modified
// source : https://github.com/glaslos/tlsh
//
...
func TestHashFilename(t *testing.T) {
	...
	tFilePath := "tests/test_file_1"
	hash, err := HashFilename(tFilePath)
	if err != nil {
		t.Error(err)
	} else {
		fmt.Println(":: ", tFilePath, " -  ", hash.String())
	}
...
func TestHash(t *testing.T) {
	...
	for _, tc := range hashTestCases {
		if hash, err := HashFilename(tc.filename); hash.String() != tc.hash {
			...
			fmt.Println("File : ", tc.filename, " -- ", hash.String())
		}
	}
...
func TestDiff(t *testing.T) {
	...
	for _, tc := range diffTestCases {
		if diff, err := DiffFilenames(tc.filenameA, tc.filenameB); diff != tc.diff {
			...
			fmt.Println("File : ", tc.filenameA, " -- ", tc.filenameB, "  :: ", diff)
		}
	}
}
